...
#define SUBLEN 10
	...
	int getSubject(char subject[MAXSJ][SUBLEN],float creditsj[MAXSJ]){
		...
		scanf("%d",&n);
		for(int i=0;i<n;i++){
			printf("Enter Name Subject %d [calculus] = ",i+1);
			scanf("%s",subject[i]);
	...
	/*-----------------------------------------------------------------------------*/
/*	void chakgr(int score[MAXSTD][MAXSJ],int nbStd,int nbSub,char id[MAXSTD][IDLEN]){
		char chakid[MAXSTD][NAMELEN];
		int count=0;
		printf("Enter number ID student = ");
		scanf("%s",&chakid[0]);
		...
		printf("\nStudent ID =%5s have gread F = %d",chakid[0],count);
	}	
	*/
...
int main(){
	...
	printf("sum student F = %d",countgr(score,nbStd,nbSub));
//	chakgr(score,nbStd,nbSub,id);
	printf("\n\nGPA : %.2f",showcd(grnum,creditsj,nbStd,nbSub,sumgpa));
}
